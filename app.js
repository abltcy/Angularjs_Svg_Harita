...
app.controller('mapController',function($scope,$rootScope){
    ...
    for (var x = 0; x <= jsn.length -1; x++) {
        for (var i = 0; i <= illerjsonx.length - 1; i++) {
            if (parseInt(jsn[x].plaka) == parseInt(illerjsonx[i].plaka)) {
                var elid = illerjsonx[i].sehirid;
                var elm = document.getElementById(elid);
                elm.style.fill = '#F72848';
                //alert(elid);
            }
    ...
    $scope.cityClick = function (plaka,il) {
        ...
        for (var i = 0; i <= jsn.length - 1; i++) {
            if (jsn[i].plaka == plaka) {
                $scope.alertmessage = il + ' Bölge Renkli';
                break;
            } else {
                $scope.alertmessage = il + ' Bölge Renksiz';
            }
        }
    }
});
